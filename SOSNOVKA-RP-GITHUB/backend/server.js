...
async function serve(req,res,url){let p=url.pathname;
  // Friendly application routes. Accept both /forum and /forum/ (and /admin) so direct navigation never falls into the static 404.
  if(p==='/'||p==='/forum'||p==='/forum/'||p==='/forum/index.html'||p==='/admin'||p==='/admin/')p='/index.html';
  if(p.startsWith('/api/'))return api(req,res,url);if(p.startsWith('/uploads/')){const rel=path.normalize(p.slice('/uploads/'.length));if(rel.startsWith('..'))return json(res,403,{error:'Forbidden'});if(await staticFile(res,path.join(UPLOADS,rel)))return;return notFound(res)}const rel=path.normalize(p.replace(/^\//,''));if(rel.startsWith('..'))return forbidden(res);if(await staticFile(res,path.join(PUBLIC,rel)))return;return notFound(res)}
